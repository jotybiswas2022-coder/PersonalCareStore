...
<style>
/* ═══════════════════════════════════════════
   LOGIN PAGE — Animated Character Edition
   ═══════════════════════════════════════════ */
...
.login-page::before {
    ...
    inset: 0;
    background:
        radial-gradient(ellipse 800px 500px at 20% 30%, rgba(37,99,235,0.08) 0%, transparent 60%),
        radial-gradient(ellipse 600px 400px at 80% 70%, rgba(124,58,237,0.05) 0%, transparent 60%);
    pointer-events: none;
}
.login-grid {
    display: grid;
...
@media (max-width: 900px) {
    .login-grid { grid-template-columns: 1fr; max-width: 460px; gap: 1.5rem; }
...
.char-panel::before {
    ...
    background: linear-gradient(135deg, rgba(96,165,250,0.2), rgba(37,99,235,0.25), rgba(124,58,237,0.2), rgba(245,158,11,0.15), rgba(96,165,250,0.2));
...
/* ── Character Base (properly proportioned) ── */
.cartoon-char {
    position: relative;
    width: 160px;
    height: 168px;
...
.char-eye {
    ...
    border-radius: 50%;
    position: relative;
    border: 1.5px solid rgba(147,197,253,0.12);
    overflow: hidden;
}
@media (max-width: 900px) {
...
.char-belly {
    ...
    background: radial-gradient(ellipse, rgba(147,197,253,0.3), rgba(59,130,246,0.08));
    border-radius: 50%;
...
@media (max-width: 900px) {
    .char-cap { width: 38px; height: 16px; top: -13px; }
    .char-cap-brim { width: 44px; height: 4px; }
    .char-cap-tassel { right: -5px; width: 3px; height: 10px; }
}
...
/* ═══════════════════════════════════════════
   CHARACTER EMOTIONS — Eyebrows, Eyes, Mouth
   ═══════════════════════════════════════════ */

/* ── Sweat Drop (nervous/shy) ── */
.char-sweat {
    position: absolute;
    top: 3px;
    right: -6px;
...
@media (max-width: 900px) {
    .char-sweat { width: 8px; height: 11px; right: -4px; }
}


/* ── EYEBROW EMOTIONS ── */

/* Raised (excited/surprised) */
...
/* Confused (one up, one down — thinking) */
...
/* Wide grin (very happy) */
.char-mouth.big-smile {
    width: 20px;
    height: 8px;
    background: transparent;
    border-bottom: 2.5px solid rgba(147,197,253,0.3);
    border-radius: 0 0 50% 50%;
    bottom: 9px;
}

/* O-shape (thinking/curious) */
.char-mouth.woah {
    width: 10px;
    height: 10px;
...
.char-mouth.nervous {
    ...
    height: 5px;
    background: transparent;
    border-bottom: 2px solid rgba(147,197,253,0.15);
    border-radius: 0 0 50% 50%;
    bottom: 10px;
...
/* Slight head tilt (thinking/curious) */
...
/* ── ARMS EMOTION EXTENSIONS ── */
.char-arm.right.point-email.excited {
    animation: armWaveFast 0.5s ease-in-out infinite;
}
@keyframes armWaveFast {
    0%,100% { transform: rotate(40deg) translateX(5px); }
    50% { transform: rotate(55deg) translateX(7px); }
}
...
.char-ambient-dots .dot:nth-child(1) { width: 8px; height: 8px; background: rgba(96,165,250,0.25); top: 15%; left: 10%; animation-delay: 0s; }
.char-ambient-dots .dot:nth-child(2) { width: 5px; height: 5px; background: rgba(167,139,250,0.2); top: 60%; left: 85%; animation-delay: 0.8s; }
.char-ambient-dots .dot:nth-child(3) { width: 6px; height: 6px; background: rgba(52,211,153,0.18); top: 80%; left: 20%; animation-delay: 1.6s; }
.char-ambient-dots .dot:nth-child(4) { width: 4px; height: 4px; background: rgba(251,191,36,0.18); top: 25%; left: 75%; animation-delay: 2.4s; }
...
.login-card .lc-header h2 { font-size: 1.375rem; font-weight: 700; color: #fff; margin-bottom: 0.25rem; }
.login-card .lc-header p { font-size: 0.8125rem; color: rgba(255,255,255,0.35); }
...
.field.focused label { color: #60A5FA; }
.field .input-wrap {
    position: relative;
}
...
.field.focused .input-wrap .input-icon { color: #60A5FA; }
.field input {
    width: 100%;
...
.remember-row .forgot-link:hover { color: #60A5FA; }
...
.btn-auth:hover { transform: translateY(-2px); box-shadow: 0 8px 24px rgba(37,99,235,0.4); }
...
.auth-divider p a:hover { color: #60A5FA; }
    ...
    <div class="login-grid">
        ...
        <div class="char-panel" id="charPanel">
            ...
            <div class="char-container">
                {{-- Pure CSS Cartoon Character (No SVG) --}}
                <div class="cartoon-char idle" id="cartoonChar">
                    ...
                    <div class="char-body" id="charBody">
                        ...
                        <div class="char-cap">
                            <div class="char-cap-brim"></div>
                            <div class="char-cap-tassel"></div>
                        </div>
                    </div>
            ...
            <div class="char-status" id="charStatus">
                <span class="greeting" id="charGreeting">Welcome Back! 👋</span>
                <span class="sub-text" id="charSubtext">I'll help you sign in</span>
            </div>
        </div>
        ...
        <div class="login-card" id="loginCard">
            ...
            <form method="POST" action="{{ route('login') }}" id="loginForm">
                ...
                <button type="submit" class="btn-auth" id="loginBtn">
                    ...
                    <span class="btn-text">
                        <svg width="16" height="16" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2.5" style="vertical-align:middle;margin-right:4px;"><path d="M15 3h4a2 2 0 012 2v14a2 2 0 01-2 2h-4M10 17l5-5-5-5M13 12H3"/></svg>
                        Sign In
                    </span>
                </button>
            </form>
...
(function() {
    ...
    function setEmotion(state) {
        ...
        switch (state) {
            ...
            case 'excited':
                ...
                charGreeting.textContent = '→ Email? Yay! I love emails! 📧✨';
                charSubtext.textContent = 'Typing away — you got this!';
                break;

            // ═══ SHY / NERVOUS (password focus) ═══
            case 'nervous':
                ...
                charGreeting.textContent = '← Password? I\'ll look away... 🙈';
                charSubtext.textContent = 'Privacy first! Your secret is safe with me';
                break;

            // ═══ THINKING (loading / checking) ═══
            case 'thinking':
                ...
                charSubtext.textContent = 'Verifying your credentials';
                break;
            ...
            case 'upset':
                ...
                charGreeting.textContent = 'Oops, that\'s not right! 😰';
                charSubtext.textContent = 'Double-check your credentials';
                break;
            ...
            default:
                ...
                charGreeting.textContent = 'Welcome Back! 👋';
                charSubtext.textContent = 'I\'ll help you sign in';
                break;
        }
    }

    // ── Look toward a field (with emotion) ──
    ...
    if (emailInput) {
        ...
        emailInput.addEventListener('blur', function() {
            ...
            if (!passwordInput?.value && !emailInput.value) lookIdle();
        });
    }
    if (passwordInput) {
        ...
        passwordInput.addEventListener('blur', function() {
            passwordField.classList.remove('focused');
            if (!emailInput?.value && !passwordInput.value) lookIdle();
        });
    }
    ...
    // ── Validation Errors ──
    if (errorAlert) {
        // First show thinking for a moment, then upset
        setEmotion('thinking');
